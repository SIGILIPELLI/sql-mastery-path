...
`SELECT` to production-grade, master-level database engineering — with
runnable queries in every module (using `sqlite3`, no server setup required)
...
- Every topic page has runnable SQL snippets — copy them into the `sqlite3`
  command-line shell (no database server to install or configure) and run
  them yourself.
- Where SQLite diverges from standard/portable SQL (used by Postgres, MySQL,
  SQL Server, etc.), the pages call it out so you know what will and won't
  carry over.
